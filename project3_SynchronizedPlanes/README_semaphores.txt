...
Unresolved:                 Our code works perfectly in cases where EMERGENCY planes do not block
                                the other runways from being used (they are correctly prioritized).
                            However, blocking the runways only works if all planes in the simulation
                                are set to be EMERGENCY planes (a mix will cause a race condition).
                            Thus, we have included two sets of code:
                                project3/sem_test:                  runs most tests
                                project3/sem_emergency_blocking:    
...
Test cases:                 Our test cases were systematic and incremental.
                            ...
                            to check that the semaphore values were as expected. Switch the
                            debug flag in header.h to switch between expected output, and
                            exhaustive debugging output (~150 lines vs ~5000 lines).
                            ...
                            -----------------------------------------------------------------------
                            Tests in project3/sem_test

                            Test 1: Default Test (output is very readable)
                            Output: out_default.txt, out_default_debug.txt


                            Test 2: Danger Zone and Planes Crashing
                            Output: out_crash.txt

                                #define FUEL_RATE 10

                            Test 3: Emergencies sort correctly (when blocking disabled).
                            Output: out_emergency.txt

                                #define FUEL_RATE 5
                                #define P_IS_EMERGENCY 0.1

                            Test 4: Introduce Randomness
                            Output: out_random.txt

                                #define T_START_RANGE 15
                                #define T_DESCEND_RANGE 3
                                #define T_LAND_RANGE 3
                                #define P_IS_EMERGENCY 0.1

                            -----------------------------------------------------------------------
                            Tests in project3/sem_emergency_blocking

                            Test 5: Emergencies properly block other runways (when all planes 
                            are emergencies)
                            Output: out_emergency_blocking.txt

                                [Just run in sem_emergency_blocking]
EOF
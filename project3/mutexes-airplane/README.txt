...
General Approach:           Each plane is represented by a separate thread passed a main 
		...
		For the mutex part:
                            ...
                            Besides the mutexes used for the Reusable Barrier, we used four
                            mutexes to solve the synchronization problem:
                            ...
                            SEM_WAIT_DONE is used by the CLEARED plane to wait until another 
                                plane begins to descend.
		 	    FREE_RUNWAY is an array of mutexes, with one mutex for each runway. Threads in the queue
                                wait on it while all the runways are full. When a plane is landing, we lock the mutex for that runway, and once it clears the runway, we clear the lock for that runway.
                            SEM_EMERGENCY is set to 1 when there is no active emergency landing,
                                and 0 when there is one. Whenever a plane thinks it is ready to
                                begin its descent, it checks the SEM_EMERGENCY value, and goes
                                back to sleep (remaining in the queue) if it equals 0.

What doesn't work:	    We were unable to get emergency planes work with blocking all the runways. Thus, we have an alternate version called new (utilizing semaphores), that shows prioritzation working, without blocking all the runways when an emergency plane is in descent or landing. We also have another version "emer-sem" that attempts to fix the problem of blocking all other runways when an emergency plane is landing. 
			    Mutliple runways for the mutex solution has some errors where some runways may be unutilized.
	  		    We have not implemented conditional variables in the mutex solution due to lack of time.		                  
